...
const LoginScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const [errors, setErrors] = useState<{mobile?: string; password?: string; general?: string}>({});
  const { setUserMobile, setUserType } = useSession();

  const handleLogin = () => {
    debugger;
    const newErrors: {mobile?: string; password?: string; general?: string} = {};
    ...
    const user = mockData.users.find(u => u.mobile === mobile && u.type === loginType);
    if (!user) {
      setErrors({ general: 'Invalid credentials. Try Customer: 9876543210 or Agent: 9123456789' });
      return;
    }
    ...
    >
      ...
      >
        <View style={styles.container}>
          {errors.general && (
            <BlurView intensity={15} style={styles.errorBanner}>
